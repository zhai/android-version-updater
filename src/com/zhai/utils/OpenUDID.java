...
public class OpenUDID {
	...
	public static void syncContext(Context mContext) {
		if (_openUdid == null) {
			...
			try {
				openContext = mContext.createPackageContext(
						"com.showcase.utils", Context.CONTEXT_IGNORE_SECURITY);
...
//		if (StringUtils.isNotBlank(_openUdid) && _openUdid.contains(":")) {
//			return _openUdid.split(":")[1];
//		}
		return _openUdid;
	}
	...
	public static String getCorpUDID(String corpIdentifier) {
		return Md5(String.format("%s.%s", corpIdentifier,
				getOpenUDIDInContext()));
	}
	...
	private static void generateOpenUDIDInContext(Context mContext) {
		...
		String _androidId = Secure.getString(mContext.getContentResolver(),
				Secure.ANDROID_ID).toLowerCase();
		if (null != _androidId && _androidId.length() > 14
				&& !_androidId.equals("9774d56d682e549c")/* android 2.2 */) {
	...
	private static void generateSystemId() {
		...
		String fp = String.format("%s/%s/%s/%s:%s/%s/%s:%s/%s/%d-%s-%s-%s-%s",
				Build.BRAND, Build.PRODUCT, Build.DEVICE, Build.BOARD,
				Build.VERSION.RELEASE, Build.ID, Build.VERSION.INCREMENTAL,
				Build.TYPE, Build.TAGS, Build.TIME, Build.DISPLAY, Build.HOST,
