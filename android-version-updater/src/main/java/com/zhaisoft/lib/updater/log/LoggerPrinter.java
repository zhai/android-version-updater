package com.zhaisoft.lib.updater.log;
...
final class LoggerPrinter implements Printer {
   ...
   *
   * @param tag is the given string which will be used in LogUtil
   */
  ...
  private int getStackOffset(StackTraceElement[] trace) {
    for (int i = MIN_STACK_OFFSET; i < trace.length; i++) {
      ...
      if (!name.equals(LoggerPrinter.class.getName()) && !name.equals(LogUtil.class.getName())) {
        return --i;
      }
    }
    return -1;
